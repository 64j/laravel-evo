...
if (!defined('SESSION_COOKIE_NAME')) {
    ...
    define('SESSION_COOKIE_NAME', env('SESSION_COOKIE_NAME', 'EVO-CMS-COMMUNITY')); // $site_sessionname
}
...
if (!defined('MODX_SITE_URL')) {
    ...
    if (!is_cli()) {
        $site_hostname = str_replace(
            ':' . $_SERVER['SERVER_PORT'],
            '',
            Arr::get($_SERVER, 'HTTP_HOST', $site_hostname)
        );
    }
    ...
    if (!in_array((int) $_SERVER['SERVER_PORT'], [80, (int) HTTPS_PORT], true) &&
        strtolower(Arr::get($_SERVER, 'HTTPS', 'off'))
    ) {
